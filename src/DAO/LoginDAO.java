...
import Model.*;
import Validaciones.Encriptador;
import java.io.*;
...
public class LoginDAO {
    ...
    public Usuario autenticarUsuario(String gmail, String idEncriptado) {
        for (Map.Entry<String, String> entry : DIRECTORIOS.entrySet()) {
            String tipoUsuario = entry.getKey();
            String directorio = entry.getValue();
            
            Usuario usuario = buscarUsuarioEnDirectorio(gmail, idEncriptado, directorio, tipoUsuario);
            if (usuario != null) {
                return usuario;
            }
        }
    ...
    private Usuario buscarUsuarioEnDirectorio(String gmail, String idEncriptado, String directorio, String tipoUsuarioDirectorio) {
            ...
            for (File archivo : archivos) {
                try (BufferedReader reader = new BufferedReader(new FileReader(archivo))) {
                    ...
                    if (gmail.equalsIgnoreCase(datos.get("email")) && 
                        idEncriptado.equals(datos.get("id"))) {
                ...
                for (File archivo : archivos) {
                    try (BufferedReader reader = new BufferedReader(new FileReader(archivo))) {
                        ...
                        if (gmail.equalsIgnoreCase(emailArchivo)) {
                            // Aquí podrías enviar el ID original si lo guardas en otro campo
                            // o implementar un sistema de recuperación diferente
                            JOptionPane.showMessageDialog(null,"Se ha enviado un recordatorio a " + gmail);
                            JOptionPane.showMessageDialog(null,"Contacte al administrador para recuperar su acceso");
                            return true;
                        }
                    } catch (IOException e) {
                        System.err.println("Error al leer archivo: " + archivo.getName());
